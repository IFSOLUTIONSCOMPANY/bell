...
export function LaundryServicePopup({
  ...
  // Articles de blanchisserie par catégorie d'après le design Figma
  const laundryItems = {
    hauts: [
  ...
  const handleConfirm = () => {
    const selectedItems: LaundryItem[] = [];
    
    Object.entries(laundryItems).forEach(([category, items]) => {
      items.forEach(item => {
  ...
  const getTotalAmount = () => {
    let total = 0;
    Object.entries(laundryItems).forEach(([, items]) => {
      items.forEach(item => {
        const quantity = quantities[item.id] || 0;
      ...
      <div className="w-full max-w-[390px] bg-[#FAF9F5] rounded-t-[50px] max-h-[80vh] overflow-y-auto shadow-lg">
        ...
        <div className="px-5">
          <div className="w-full h-48 bg-gray-100 rounded-2xl mb-5 overflow-hidden">
            <img 
              src="https://images.unsplash.com/photo-1517677208171-0bc6725a3e60?fit=crop&w=400&h=300"
                      ...
                      {quantities[item.id] > 0 ? (
                        ...
                        >
                          <svg width="12" height="12" viewBox="0 0 12 12" fill="none">
                            <path d="M6 1V11M1 6H11" stroke="#65413D" strokeWidth="2" strokeLinecap="round"/>
                          </svg>
                        </button>
                      )}
                    </div>
